...
public class CollectorCoordinator {
	private static Logger log = Logger.getLogger(CollectorCoordinator.class);
	String name;
	String type;
	...
	public void stop() {
		...
		config.put(member.getName(), "STOP");
		myConfigStore.put(config);
	}
	
	public void warning() {
		HashMap<String, Object> config = (HashMap<String, Object>) myConfigStore.get();
		config.put(member.getName(), "WARNING");
		myConfigStore.put(config);
	...
	public void recover() {
		HashMap<String, Object> config = (HashMap<String, Object>) myConfigStore.get();
		config.put(member.getName(), "START");
		myConfigStore.put(config);
	}
		...
		public void onMemberLeft(List<MemberInfo> members) {
			...
			for (MemberInfo member : members) {
				...
				if (status.equals("START")) {
					log.info("Change status START -> LOST");
					HashMap<String, Object> config = (HashMap<String, Object>) myConfigStore.get();
					config.put(member.getName(), "LOST");
					myConfigStore.put(config);
				} 
			}
		}
		...
		public void run() {
			while (true) {
				try {
					...
					List<MemberInfo> liveMembers = myGMS.getMembers();
					log.info("Live Node " + liveMembers.size());
					List<MemberInfo> memberInfos = myGMS.getMembers(true);

					
					boolean isLive = false;
					for(MemberInfo member: memberInfos) {
						for(MemberInfo liveMember : liveMembers) {
							if (liveMember.getName() == member.getName()) {
								config.put(member.getName(), "LOST");
								break;
							}
						}
